...
/*   Updated: 2022/12/06 21:27:56 by hwichoi          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	init_mutex(t_desk *desk)
{
	...
	if (!desk -> forks)
		return (0);
	desk -> check_mutex = malloc(sizeof(pthread_mutex_t) * desk -> phils_num);
	if (!desk -> forks)
		return (0);
	i = 0;
	while (i < desk -> phils_num)
	{
		...
		pthread_mutex_init(&(desk -> check_mutex[i++]), NULL);
	}
