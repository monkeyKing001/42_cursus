...
/*   Updated: 2022/12/06 21:39:06 by hwichoi          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	eating(t_desk *desk, int phil_idx)
{
	...
	if (check_die_desk(desk, phil_idx, CHECK) == 0)
	{
		...
		pthread_mutex_lock(&(desk -> check_mutex[phil_idx]));
		phil -> num_eat++;
		phil -> last_time = get_time_ms();
		pthread_mutex_unlock(&(desk -> check_mutex[phil_idx]));
